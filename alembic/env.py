...
# --- Make the app importable and pull in every domain's models so
# their tables register on SQLModel.metadata before autogenerate runs.
...
from app.src.payments import models as payments_models  # noqa: F401
...
# Drive the DB URL from app settings/.env instead of a hardcoded value
# in alembic.ini, so dev/staging/prod all use the same source of truth.
